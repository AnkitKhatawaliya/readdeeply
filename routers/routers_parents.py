...
from database import db_validate_parent, db_get_student_info

router = APIRouter()

@router.get("/validate_parent/{standard}/{section}/{roll_number}/{password}", status_code=status.HTTP_200_OK)
def validate_parent_login(standard: str, section: str, roll_number: int, password: str):
    is_valid = db_validate_parent(standard, section, roll_number, password)
    if is_valid:
